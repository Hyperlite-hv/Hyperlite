...
export default function DriversIsoControl({ vmName, onChanged }) {
  ...
  return (
    <div className="space-y-2 border-t border-anthracite-600 px-4 py-3">
      ...
      <select id="vm-driver-iso" className="input" value={iso} onChange={(e) => setIso(e.target.value)}>
        <option value="">Select a driver ISO</option>
        {isos.map((item) => <option key={item.nom} value={item.nom}>{item.nom}</option>)}
      </select>
      <div className="flex gap-2">
        <button className="btn-secondary" disabled={busy || !iso} onClick={() => changeMedia(false)}>Mount drivers</button>
        <button className="btn-secondary" disabled={busy} onClick={() => changeMedia(true)}>Eject drivers</button>
      </div>
