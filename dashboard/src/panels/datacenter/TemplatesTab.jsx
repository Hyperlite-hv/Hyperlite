...
export default function TemplatesTab() {
  ...
  async function handleDeploy() {
    ...
    try {
      ...
      pushToast({ kind: "success", title: "VM déployée", message: newName.trim() });
      setDeployTarget(null);
      setNewName("");
      await loadAll();
    } catch (e) {
      pushToast({ kind: "error", title: "Échec du déploiement", message: e.message });
  ...
  async function handleDelete() {
    ...
    try {
      ...
      pushToast({ kind: "success", title: "Template supprimé", message: pendingDelete.nom });
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-anthracite-400">Convertir une VM (arrêtée) en template se fait depuis son onglet Résumé.</p>
          ...
          <div key={t.nom} className="flex items-center gap-3 px-4 py-2.5 text-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-anthracite-400 truncate">
                depuis {t.vm_source} -- {t.vcpu} vCPU / {t.memoire_mo} Mo -- créé par {t.cree_par} le {t.cree_le}
              </div>
            </div>
            {isAdmin && (
              <>
                <button className="btn-secondary" disabled={busy} onClick={() => { setDeployTarget(t); setNewName(`${t.nom}-01`); }}>
                  <Rocket size={13} /> Déployer
                </button>
      ...
      {deployTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setDeployTarget(null)}>
          <div className="card w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-anthracite-100 mb-3">Déployer "{deployTarget.nom}"</h3>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button className="btn-primary" disabled={busy || !newName.trim()} onClick={handleDeploy}>Déployer</button>
            </div>
          </div>
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        message="Le disque du template sera définitivement supprimé."
