...
// Reel : GET /backups (toutes VM confondues, voir app/routers/backups.py).
export default function BackupsTab() {
  const pushToast = useInfraStore((s) => s.pushToast);
  const [rows, setRows] = useState(null);

  useEffect(() => {
    fetchAllBackups().then(setRows).catch((e) => pushToast({ kind: "error", title: "Erreur backups", message: e.message }));
  }, [pushToast]);
  ...
  if (rows.length === 0) {
    return (
      <div className="card flex flex-col items-center gap-2 p-8 text-center">
        ...
        <p className="text-sm text-anthracite-300">Aucune sauvegarde pour le moment.</p>
        <p className="text-xs text-anthracite-500 max-w-sm">
          Lancez une sauvegarde manuelle ou planifiez-en une depuis l'onglet Sauvegarde d'une VM.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="card divide-y divide-anthracite-600 max-h-[70vh] overflow-y-auto">
      <div className="grid grid-cols-[110px_1fr_80px_90px_1fr_50px] gap-2 px-4 py-2 text-xs font-medium text-anthracite-400 sticky top-0 bg-anthracite-800">
        <span>Heure</span><span>VM</span><span>Mode</span><span>Taille</span><span>Emplacement / cause</span><span>Statut</span>
      </div>
      {rows.map((b) => (
        <div key={b.id} className="grid grid-cols-[110px_1fr_80px_90px_1fr_50px] gap-2 px-4 py-2 text-sm items-center">
          <span className="text-anthracite-400 text-xs font-mono">{new Date(b.cree_le).toLocaleString("fr-FR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</span>
          <span className="text-anthracite-100 truncate">{b.vm_name}</span>
          <span className="text-xs text-anthracite-400">{b.mode}</span>
