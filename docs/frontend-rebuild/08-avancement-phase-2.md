...
## 10. Suite (2026-09-26) : publication sur `test`, arbre retiré, pages VM et nœud

- Publication : le code est fusionné sur la branche `test` par PR (#173, #176, #177, puis celle de ce lot), CI verte à chaque fois. L'instance de dev est redéployée depuis la tête de `test` uniquement (backend et frontend), jamais depuis un build local non poussé.
- Arbre d'inventaire retiré : la barre latérale suit la maquette (sections Infrastructure, Protection, Opérations, Administration). Nœuds et VM se trouvent par leurs pages, la recherche (Ctrl K) et le sélecteur de cluster.
- Mise en page : une seule grille et un seul espacement pour toutes les pages, cartes d'une même rangée à la même hauteur, barres de défilement fines aux couleurs du thème.
- Page d'une VM, sur le modèle de la maquette : onglets Résumé, Performances, Snapshots, Sauvegardes, Matériel (avec Options), Réseau, Console. Le Résumé montre la configuration réelle (disques, interfaces, protection) à gauche, les quatre graphiques de la dernière heure (CPU, mémoire, E/S disque, réseau) et les derniers snapshots à droite. L'onglet Performances ajoute la période (1h, 24h, 7j, 30j) et les min / moyenne / max. En-tête : Console ou Démarrer, Snapshot, Migrer… (fenêtre avec diagnostic de compatibilité), Arrêter (avec confirmation) ; les actions indisponibles restent visibles et disent pourquoi. Tous les anciens liens `?tab=` restent valides.
- Page d'un nœud, même structure : onglets à plat (Résumé, Performances, Résumé système, Réseau, Stockage disque, Tâches, Compatibilité, Shell), configuration et alertes à gauche, graphiques CPU et mémoire de l'hôte, VM du nœud et activité à droite ; en-tête Shell et Nouvelle VM.
- Corrections trouvées en route : date de création des snapshots affichée en horodatage Unix brut (maintenant en date lisible), liste des réseaux en erreur 500 si un réseau disparaît pendant la lecture, métriques d'une VM en erreur 500 si elle s'arrête pendant l'échantillonnage.
- Toujours « Non fourni » car absent de l'API : ID numérique d'une VM, espace utilisé dans le disque invité, taille des snapshots, métriques des nœuds distants, débit disque et réseau de l'hôte.
