...
- Secrets of the `release` environment: `GPG_PRIVATE_KEY` (the armored signing key),
  `GPG_PASSPHRASE` (only if the key has one) and the credentials of the mirror, either `MIRROR_DEPLOY_KEY`
  (private half of a deploy key with write access to the mirror repository; preferred, needs deploy keys
  to be enabled for the organization) or `MIRROR_TOKEN` (a fine-grained personal access token limited to
  the mirror repository, *Contents: read and write*; it expires, note the date and renew it).
